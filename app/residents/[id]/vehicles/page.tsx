...
export default async function ResidentVehicleListPage({ params }: ResidentVehicleListPageProps) {
      ...
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-col gap-4 rounded-lg border border-cyan-950/10 bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              {resident.vehicles.length} vehicle{resident.vehicles.length !== 1 ? "s" : ""} for {resident.name}
            </p>
          </div>
          <Link className="inline-flex min-h-11 items-center gap-2 rounded-md bg-cyan-700 px-4 text-sm font-semibold text-white transition hover:bg-cyan-800" href={`/residents/${residentId}/vehicles/new`}>
        ...
        {resident.vehicles.length === 0 ? (
          <p className="mt-8 rounded-lg border border-cyan-950/10 bg-white p-8 text-center text-sm text-slate-500">
            No vehicles recorded for this resident.
          </p>
        ) : (
