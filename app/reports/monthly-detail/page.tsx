...
import { ChevronRight, FileSpreadsheet, FileText, Home, TableProperties } from "lucide-react";
    ...
    <main className="min-h-screen bg-[var(--background)] px-4 py-6 text-slate-950 sm:px-6 lg:px-8">
      <div className="mx-auto grid w-full max-w-7xl gap-6 [&>*]:min-w-0">
        <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm text-slate-500">
          ...
          <span className="font-medium text-slate-700">Monthly Detail</span>
        </nav>
        <header className="ui-card flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-brand-700">Monthly detail</p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <a className="ui-button-secondary" href={`/reports/monthly.xlsx?${exportQuery}`}>
              <FileSpreadsheet aria-hidden="true" size={17} />
              Excel
            </a>
            <a className="ui-button-secondary" href={`/reports/monthly.pdf?${exportQuery}`}>
            ...
            <table className="w-full min-w-220 border-collapse text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  ...
                  <th className="sticky top-0 bg-slate-50 px-5 py-3 font-semibold">Paid until</th>
                  <th className="sticky top-0 bg-slate-50 px-5 py-3 font-semibold">{monthLabel(selectedYear, selectedMonth)}</th>
                  <th className="sticky top-0 bg-slate-50 px-5 py-3 font-semibold">Outstanding</th>
                  <th className="sticky top-0 bg-slate-50 px-5 py-3 font-semibold">Extra</th>
                  <th className="sticky top-0 bg-slate-50 px-5 py-3 font-semibold">Last payment</th>
                </tr>
              </thead>
