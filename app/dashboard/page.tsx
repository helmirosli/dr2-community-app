      ...
      <div className="mx-auto grid w-full max-w-7xl gap-8 [&>*]:min-w-0">
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {metrics.map((metric) => {
            ...
            return (
              <div className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm" key={metric.label}>
        ...
        <section className="grid gap-8 xl:grid-cols-[1fr_320px]">
          ...
          <div className="rounded-xl border border-border-subtle bg-white shadow-sm overflow-hidden">
            <div className="flex flex-col gap-3 border-b border-slate-100 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button className="inline-flex items-center gap-2 rounded-lg border border-border-subtle bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2" type="button">
                <Search size={16} />
                {t.common.filter}
              </button>
            </div>
              ...
              <div className="lg:hidden px-4 py-3 space-y-3">
                {recentSubmissions.length > 0 ? (
                  recentSubmissions.map((submission) => (
                    <div key={submission.id} className="rounded-lg border border-slate-100 bg-white p-3 shadow-sm">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-2 text-sm text-slate-600">{monthLabel(submission.coverageStartYear, submission.coverageStartMonth)} – {monthLabel(submission.coverageEndYear, submission.coverageEndMonth)}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-slate-900">{formatRM(submission.amountSen)}</p>
                          <p className="mt-1 text-xs inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-2 py-1 text-amber-700">{t.dashboard.review}</p>
                        </div>
                      </div>
                      <div className="mt-3 flex items-center justify-between gap-3">
                        <FileViewer files={submission.uploads} />
                        <div className="flex gap-2">
                          <ApproveRejectButton action={approveSubmission.bind(null, submission.id)} label={<><CheckCircle2 size={14} />{t.common.approve}</>} className="inline-flex items-center justify-center gap-1.5 rounded-md bg-emerald-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-60" />
                          <ApproveRejectButton action={rejectSubmission.bind(null, submission.id)} label={<><XCircle size={14} />{t.common.reject}</>} className="inline-flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-red-200 hover:bg-red-50 hover:text-red-700 disabled:opacity-60" />
                        </div>
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="text-center text-slate-500">{t.dashboard.noPending}</div>
                )}
              </div>
        ...
        <section className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            <Link className="rounded-xl border border-border-subtle bg-white p-5 shadow-sm transition hover:shadow-md" href={href} key={title}>
              <div className="flex items-start justify-between gap-4">
